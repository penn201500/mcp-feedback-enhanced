...
class FeedbackApp {
    ...
    initImageElements() {
        // 根據當前佈局模式確定元素前綴
        const prefix =
            this.layoutMode && this.layoutMode.startsWith("combined") ? "combined" : "feedback"

        console.log(`🖼️ 初始化圖片元素，使用前綴: ${prefix}`)

        // 圖片相關元素 - 優先使用當前模式的元素
        this.imageInput =
            document.getElementById(`${prefix}ImageInput`) || document.getElementById("imageInput")
        this.imageUploadArea =
            document.getElementById(`${prefix}ImageUploadArea`) ||
            document.getElementById("imageUploadArea")
        this.imagePreviewContainer =
            document.getElementById(`${prefix}ImagePreviewContainer`) ||
            document.getElementById("imagePreviewContainer")
        this.imageSizeLimitSelect =
            document.getElementById(`${prefix}ImageSizeLimit`) ||
            document.getElementById("imageSizeLimit")
        this.enableBase64DetailCheckbox =
            document.getElementById(`${prefix}EnableBase64Detail`) ||
            document.getElementById("enableBase64Detail")
        ...
        if (!this.imageInput || !this.imageUploadArea) {
            console.warn(
                `⚠️ 圖片元素初始化失敗 - imageInput: ${!!this.imageInput}, imageUploadArea: ${!!this
                    .imageUploadArea}`
            )
        } else {
    ...
    setupImageEventListeners() {
        console.log(
            `🖼️ 設置圖片事件監聽器 - imageInput: ${this.imageInput?.id}, imageUploadArea: ${this.imageUploadArea?.id}`
        )

        // 文件選擇事件
        this.imageChangeHandler = e => {
            console.log(
                `📁 文件選擇事件觸發 - input: ${e.target.id}, files: ${e.target.files.length}`
            )
    ...
    initImageHandling() {
        ...
        console.log(
            `🔍 檢查圖片元素 - imageUploadArea: ${
                this.imageUploadArea?.id || "null"
            }, imageInput: ${this.imageInput?.id || "null"}`
        )

        if (!this.imageUploadArea || !this.imageInput) {
            console.warn(
                `⚠️ 圖片處理初始化失敗 - imageUploadArea: ${!!this
                    .imageUploadArea}, imageInput: ${!!this.imageInput}`
            )
            return
        }
                ...
                base64Checkbox.addEventListener("change", e => {
                    ...
                    prefixes.forEach(otherPrefix => {
                        const otherCheckbox = document.getElementById(
                            `${otherPrefix}EnableBase64Detail`
                        )
    ...
    canSubmitFeedback() {
        ...
        console.log(
            `🔍 檢查提交權限: feedbackState=${this.feedbackState}, isConnected=${this.isConnected}, connectionReady=${this.connectionReady}, canSubmit=${canSubmit}`
        )
        return canSubmit
    }
        ...
        if (this.submitBtn) {
            ...
            switch (this.feedbackState) {
                case "waiting_for_feedback":
                    this.submitBtn.textContent = window.i18nManager
                        ? window.i18nManager.t("buttons.submit")
                        : "提交回饋"
                    this.submitBtn.className = "btn btn-primary"
                    break
                case "processing":
                    this.submitBtn.textContent = window.i18nManager
                        ? window.i18nManager.t("buttons.processing")
                        : "處理中..."
                    this.submitBtn.className = "btn btn-secondary"
                    break
                case "feedback_submitted":
                    this.submitBtn.textContent = window.i18nManager
                        ? window.i18nManager.t("buttons.submitted")
                        : "已提交"
    ...
    updateStatusIndicator() {
        ...
        switch (this.feedbackState) {
            case "waiting_for_feedback":
                icon = "⏳"
                title = window.i18nManager
                    ? window.i18nManager.t("status.waiting.title")
                    : "等待回饋"
                message = window.i18nManager
                    ? window.i18nManager.t("status.waiting.message")
                    : "請提供您的回饋意見"
                status = "waiting"
                break
            ...
            case "processing":
                icon = "⚙️"
                title = window.i18nManager
                    ? window.i18nManager.t("status.processing.title")
                    : "處理中"
                message = window.i18nManager
                    ? window.i18nManager.t("status.processing.message")
                    : "正在提交您的回饋..."
                status = "processing"
                break
            ...
            case "feedback_submitted":
                ...
                icon = "✅"
                title = window.i18nManager
                    ? window.i18nManager.t("status.submitted.title")
                    : "回饋已提交"
                message = window.i18nManager
                    ? window.i18nManager.t("status.submitted.message")
                    : "等待下次 MCP 調用"
                if (timeStr) {
                    message += ` (${timeStr})`
                }
            ...
            this.websocket.onclose = event => {
                ...
                if (event.code === 4004) {
                    ...
                    // 只有在非正常關閉時才重連
                        ...
                        console.log(
                            `${this.reconnectDelay / 1000}秒後嘗試重連... (第${
                                this.reconnectAttempts
                            }次)`
                        )
                        setTimeout(() => {
    ...
    canSubmitFeedback() {
        ...
        console.log(
            `🔍 檢查提交權限: isConnected=${this.isConnected}, connectionReady=${this.connectionReady}, feedbackState=${this.feedbackState}, canSubmit=${canSubmit}`
        )
        return canSubmit
    }
    ...
    resetFeedbackForm() {
        ...
        const submitButtons = document.querySelectorAll(
            ".submit-button, #submitButton, #combinedSubmitButton"
        )
    ...
    updateStatusIndicators() {
        ...
        // 使用國際化系統獲取翻譯文字
        const waitingTitle = window.i18nManager
            ? window.i18nManager.t("status.waiting.title")
            : "Waiting for Feedback"
        const waitingMessage = window.i18nManager
            ? window.i18nManager.t("status.waiting.message")
            : "Please provide your feedback on the AI work results"
        ...
        // 更新合併模式的狀態指示器
        const combinedFeedbackStatusIndicator = document.getElementById(
            "combinedFeedbackStatusIndicator"
        )
    ...
    handleStatusUpdate(statusInfo) {
        ...
        switch (statusInfo.status) {
            case "feedback_submitted":
                ...
                this.updateSummaryStatus("已送出反饋，等待下次 MCP 調用...")
                const submittedConnectionText = window.i18nManager
                    ? window.i18nManager.t("connection.submitted")
                    : "已連接 - 反饋已提交"
            ...
            case "waiting":
                ...
                const waitingConnectionText = window.i18nManager
                    ? window.i18nManager.t("connection.waiting")
                    : "已連接 - 等待回饋"
    ...
    disableSubmitButton() {
        ...
        if (submitBtn) {
            ...
            submitBtn.textContent = window.i18nManager
                ? window.i18nManager.t("buttons.submitted")
                : "✅ 已提交"
    ...
    clearFeedback() {
        ...
        submitButtons.forEach(button => {
            button.disabled = false
            button.textContent = window.i18nManager
                ? window.i18nManager.t("buttons.submit")
                : "提交回饋"
        })
    ...
    focusInput() {
        ...
        if (this.layoutMode.startsWith("combined")) {
            // 工作區模式：聚焦合併模式的輸入框
            targetInput = document.getElementById("combinedFeedbackText")
        } else {
            // 分離模式：聚焦回饋分頁的輸入框
            targetInput = document.getElementById("feedbackText")
        }
     ...
     */
    fixPlaceholderText() {
        console.log("🔧 開始修復 placeholder 文本換行...")

        // 確保 CSS 樣式存在
        this.ensurePlaceholderStyles()

        const textareas = ["feedbackText", "combinedFeedbackText"]

        textareas.forEach(id => {
            const textarea = document.getElementById(id)
            if (textarea) {
                // 獲取當前語言的詳細提示信息
                const detailedPlaceholder = window.i18nManager
                    ? window.i18nManager.t("feedback.detailedPlaceholder")
                    : "請在這裡輸入您的回饋... 💡 提示：Ctrl+Enter提交 | Ctrl+I聚焦 | Ctrl+V貼圖"

                // 方法1: 直接設置 placeholder 屬性
                textarea.placeholder = detailedPlaceholder

                // 方法2: 強制更新 DOM 屬性
                textarea.setAttribute("placeholder", detailedPlaceholder)

                // 方法3: 觸發重新渲染
                textarea.style.display = "none"
                textarea.offsetHeight // 強制重新計算
                textarea.style.display = ""

                console.log(
                    `✅ 已設置 ${id} 的 placeholder:`,
                    detailedPlaceholder.substring(0, 50) + "..."
                )
                console.log(
                    `🔍 實際 placeholder 值:`,
                    textarea.placeholder.substring(0, 50) + "..."
                )
            } else {
        ...
        console.log("✅ Placeholder 文本換行修復完成")
    }
    ...
    ensurePlaceholderStyles() {
        // 移除舊的樣式（如果存在）
        const existingStyle = document.getElementById("placeholder-fix-style")
        if (existingStyle) {
        ...
        style.id = "placeholder-fix-style"
        style.textContent = `
            /* 多瀏覽器兼容的 placeholder 樣式 */
        ...
        console.log("✅ Placeholder CSS 樣式已重新添加（增強版）")
    }
        ...
        try {
            ...
            if (settings) {
                ...
                if (settings.language && window.i18nManager) {
                    ...
                    console.log(
                        `檢查語言設定: ui_settings.json=${settings.language}, i18nManager=${currentI18nLanguage}`
                    )
                    if (settings.language !== currentI18nLanguage) {
                        console.log(
                            `🔄 同步語言設定: ${currentI18nLanguage} -> ${settings.language}`
                        )
    ...
    syncImageContent() {
        ...
        const combinedImagePreviewContainer = document.getElementById(
            "combinedImagePreviewContainer"
        )
    ...
    async checkForSessionUpdate() {
        try {
            ...
            if (sessionData.session_id && sessionData.session_id !== this.lastKnownSessionId) {
                console.log(
                    `🔄 檢測到新會話: ${this.lastKnownSessionId} -> ${sessionData.session_id}`
                )
    ...
    updateAutoRefreshStatus(status = null) {
        ...
        console.log(
            `🔄 自動檢測狀態翻譯: ${textKey} -> ${translatedText} (語言: ${window.i18nManager.currentLanguage})`
        )
