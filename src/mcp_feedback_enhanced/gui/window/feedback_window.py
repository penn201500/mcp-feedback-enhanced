...
class FeedbackWindow(QMainWindow):
    ...
    def __init__(self, project_dir: str, summary: str):
        ...
        # 新增：窗口行為配置
        self.auto_focus = self.config_manager.get_setting('window.auto_focus', True)  # 自動聚焦
        self.stay_on_top = self.config_manager.get_setting('window.stay_on_top', False)  # 保持最前面
        self.auto_raise = self.config_manager.get_setting('window.auto_raise', True)  # 自動置頂
        self.minimize_on_focus_loss = self.config_manager.get_setting('window.minimize_on_focus_loss', False)  # 失去焦點時最小化
    ...
    def _apply_window_behavior(self) -> None:
        ...
        # 自動聚焦和置頂
        if self.auto_focus or self.auto_raise:
            # 使用 QTimer 確保在窗口完全創建後執行
            QTimer.singleShot(100, self._force_window_focus)

        debug_log(f"窗口行為設置完成 - 自動聚焦: {self.auto_focus}, 保持最前面: {self.stay_on_top}, 自動置頂: {self.auto_raise}")

    def _force_window_focus(self) -> None:
        """強制窗口獲得焦點"""
        if self.auto_raise:
            self.raise_()  # 將窗口置於最前面
        ...
        # 自動聚焦到回饋輸入框
        self._focus_feedback_input()

        debug_log("窗口已強制獲得焦點")

    def _focus_feedback_input(self) -> None:
        """聚焦到回饋輸入框"""
        try:
            # 使用 QTimer 延遲執行，確保窗口完全初始化後再聚焦輸入框
            from PySide6.QtCore import QTimer
            QTimer.singleShot(200, self._delayed_focus_input)
        except Exception as e:
            debug_log(f"聚焦輸入框時發生錯誤: {e}")

    def _delayed_focus_input(self) -> None:
        """延遲聚焦輸入框"""
        try:
            # 查找回饋分頁
            if hasattr(self, 'tab_manager') and self.tab_manager.feedback_tab:
                self.tab_manager.feedback_tab.focus_input()
                debug_log("已自動聚焦到回饋輸入框")
            else:
                debug_log("未找到回饋分頁，無法聚焦輸入框")
        except Exception as e:
            debug_log(f"延遲聚焦輸入框時發生錯誤: {e}")

    def _setup_window_shortcuts(self) -> None:
        """設置窗口相關的快捷鍵"""
        # Ctrl+Shift+F - 切換窗口保持最前面狀態
        toggle_on_top_shortcut = QShortcut(QKeySequence("Ctrl+Shift+T"), self)
        toggle_on_top_shortcut.activated.connect(self._toggle_stay_on_top)

        # Ctrl+Shift+R - 重新聚焦窗口
        refocus_shortcut = QShortcut(QKeySequence("Ctrl+Shift+R"), self)
        refocus_shortcut.activated.connect(self._force_window_focus)

        # Ctrl+Shift+H - 隱藏/顯示窗口
        hide_show_shortcut = QShortcut(QKeySequence("Ctrl+Shift+H"), self)
        hide_show_shortcut.activated.connect(self._toggle_window_visibility)
    ...
    def _toggle_stay_on_top(self) -> None:
        ...
        self.config_manager.set_setting('window.stay_on_top', self.stay_on_top)
        ...
        debug_log(f"窗口保持最前面狀態已切換為: {self.stay_on_top}")
    ...
    def focusOutEvent(self, event) -> None:
        ...
        if self.minimize_on_focus_loss:
            # 延遲最小化，避免在快速切換時誤觸發
            QTimer.singleShot(2000, self._delayed_minimize)

    def _delayed_minimize(self) -> None:
        """延遲最小化窗口"""
        if not self.hasFocus() and self.minimize_on_focus_loss:
            self.showMinimized()
            debug_log("窗口因失去焦點而最小化")
